...
public class BTreeFile implements DbFile {
	...
	private BTreeLeafPage findLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreePageId pid, Permissions perm,
		...
		BTreeEntry entry = it.next();
 		BTreePageId searchId = null;
 		if (f == null)
			searchId = entry.getLeftChild();
 		else
		 {
 			...
 			if (f.compare(Op.LESS_THAN_OR_EQ, entry.getKey()))
			{
			 ...
			 {
 				searchId = entry.getRightChild();
			}
		}
        return findLeafPage(tid, dirtypages, searchId, perm, f);
	}
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field) 
		...
		BTreeLeafPage newRLeaf = (BTreeLeafPage) getEmptyPage(tid, dirtypages, BTreePageId.LEAF);
		Iterator<Tuple> it = page.reverseIterator();
		int numTuples = page.getNumTuples();
		int len = numTuples - numTuples / 2;
		Tuple [] tupleToMove = new Tuple[len];
		for (int i = len - 1; i >= 0; i--)
			tupleToMove[i] = it.next();
		for (int i = len - 1; i >= 0; i--)
		{
			page.deleteTuple(tupleToMove[i]);
			newRLeaf.insertTuple(tupleToMove[i]);
		}


		
		BTreePageId oldRightSibId = page.getRightSiblingId();
		newRLeaf.setLeftSiblingId(page.getId());
		newRLeaf.setRightSiblingId(oldRightSibId);
		page.setRightSiblingId(newRLeaf.getId());
		if (oldRightSibId != null)
		 {
			BTreeLeafPage oldrightPage = (BTreeLeafPage) getPage(tid, dirtypages, oldRightSibId, Permissions.READ_WRITE);
			oldrightPage.setLeftSiblingId(newRLeaf.getId());
		}

		//Update Parents
		Tuple tuple = it.next();
		Field midkey = tuple.getField(keyField);
		BTreeEntry midEntry = new BTreeEntry(midkey, page.getId(), newRLeaf.getId());
		...
		updateParentPointer(tid, dirtypages, parent.getId(), newRLeaf.getId());
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int len = numEntries / 2;
		BTreeEntry [] entryToMove = new BTreeEntry[len];
		for (int i = len - 1; i >= 0; i--)
			entryToMove[i] = it.next();

		BTreeEntry midEntry = it.next();
		for (int i = len - 1; i >= 0; i--)
		{
			page.deleteKeyAndRightChild(entryToMove[i]);
			newInternal.insertEntry(entryToMove[i]);
		}
		page.deleteKeyAndRightChild(midEntry);

		//Update the  parent
		Field midkey = midEntry.getKey();
		midEntry = new BTreeEntry(midkey, page.getId(), newInternal.getId());
		...
		parent.insertEntry(midEntry);
		updateParentPointer(tid, dirtypages, parent.getId(), page.getId());
		updateParentPointer(tid, dirtypages, parent.getId(), newInternal.getId());
		if (midkey.compare(Op.GREATER_THAN_OR_EQ, field)) 
			return page;
		else 
			return newInternal;
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		BTreeEntry midEntry = entryTomove[numTomove - 1];
		Field newParentKey = midEntry.getKey();
		Field oldParentKey = parentEntry.getKey();
		parentEntry.setKey(newParentKey);
		parent.updateEntry(parentEntry);

		page.insertEntry(new BTreeEntry(oldParentKey, LeftChild, RightChild));
		for (int i = 0; i < numTomove - 1; i++)
		{
			leftSibling.deleteKeyAndRightChild(entryTomove[i]);
			page.insertEntry(entryTomove[i]);
		}
		leftSibling.deleteKeyAndRightChild(entryTomove[numTomove - 1]);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		for (int i = 0; i < numTomove - 1; i++)
		{
			rightSibling.deleteKeyAndRightChild(entryTomove[i]);
			page.insertEntry(entryTomove[i]);
		}
		rightSibling.deleteKeyAndLeftChild(entryTomove[numTomove - 1]);
		updateParentPointers(tid, dirtypages, page);
	}
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numToDelete = rightPage.getNumTuples();
		Tuple[] tuplesToDelete = new Tuple[numToDelete];
		Iterator<Tuple> it = rightPage.iterator();
		for (int i = 0; i < numToDelete; i++)
			tuplesToDelete[i] = it.next();
	...
	protected void mergeInternalPages(TransactionId tid, HashMap<PageId, Page> dirtypages, 
		...
		int numToDelete = rightPage.getNumEntries();
		BTreeEntry[] entryToDelete = new BTreeEntry[numToDelete];
		Iterator<BTreeEntry> it = rightPage.iterator();
		for (int i = 0; i < numToDelete; i++)
			entryToDelete[i] = it.next();

		for (BTreeEntry entry : entryToDelete)
		{
